...
import { INewsCategoryFindAllOptions } from "./interface";

import { NewsCategory } from "../models";
import { getUserFieldDescriptions } from "../models/NewsCategory";
...
class NewsCategoryController {
  ...
  constructor() {
    this.module = "Categoria da Notícia";
  ...
  public async create(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const create = await NewsCategory.create({ label, url, color  });
  ...
  public async getAll(req: Request, res: Response): Promise<Response> {
    try {
      const options: INewsCategoryFindAllOptions = {};
      ...
      const findAll = await NewsCategory.findAll(options);
  ...
  public async getById(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const findByPk = await NewsCategory.findByPk(uuid);
  ...
  public async update(req: Request, res: Response): Promise<Response> {
    try {
      ...
      await findByPk.update({ label, url, color  });
      return res.status(200).json(findByPk);
